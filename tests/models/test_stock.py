...
class StockTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_trade_price_limit_flags_ex(self):
        limits = np.array(
            [
                (
                    datetime.date(2022, 3, 22),
        ...
        await Stock.save_trade_price_limits(limits, False)
        actual = await Stock.trade_price_limit_flags_ex(code, start, end)
        self.assertEqual(len(actual), 9)
        self.assertTupleEqual(actual[datetime.date(2022, 3, 22)], (True, False))
    ...
    async def test_get_buy_limit_secs(self):
        limits = np.array(
            [
                (
                    datetime.date(2022, 3, 22),
